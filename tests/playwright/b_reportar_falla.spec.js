...
test('CP005 - Crear reporte de falla para una máquina existente', async ({ page }) => {
  ...
  // 2. Seleccionar la máquina creada anteriormente (TAB-03)
  // Forzamos a que busque la celda específica por su nombre/código exacto
  const celdaMaquina = page.getByRole('cell', { name: 'TAB-03' }).or(page.getByText('TAB-03'));
  ...
  await expect(page.getByRole('heading', { name: /TAB-03/i })).toBeVisible({ timeout: 5000 });
  ...
  // Verificamos que la orden asociada a la máquina TAB-03 sea visible en el listado general
  await expect(page.getByRole('heading', { name: /TAB-03/i }).or(page.getByText('TAB-03'))).toBeVisible({ timeout: 10000 });
});