...
import { usePreventive, type PreventiveMaintenance } from "../hooks/usePreventive";
...
const PreventivePage = () => {
  ...
  if (isLoading) {
    return (
      <div className="p-6 flex justify-center items-center h-full">
        <p className="text-gray-600 font-semibold animate-pulse">Cargando mantenimientos programados...</p>
      </div>
    );
  }
  ...
  const columns = [
    { 
      header: "Máquina", 
      // Pasamos una función al accessor para extraer el nombre
      accessor: (row: PreventiveMaintenance) => row.machine?.name || "Desconocida",
    },
    { 
    ...
    { 
      ...
      accessor: (row: PreventiveMaintenance) => new Date(row.nextDate).toLocaleDateString() 
    },
    { 
      header: "Estado", 
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Mantenimiento Preventivo</h1>
          <p className="text-gray-500 text-sm mt-1">Gestiona las tareas programadas para los equipos.</p>
        </div>
