...
const AddEmployeeModal = ({ isOpen, onClose, onSuccess }: Props) => {
  const { createEmployee, isSubmitting } = useEmployees();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onClose();
      alert(
        "Empleado creado exitosamente. Se le ha enviado un correo con su contraseña.",
      );
    } catch (error: any) {
      alert(error.message);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-md rounded-lg shadow-2xl border border-gray-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="bg-blue-50 text-blue-800 p-3 rounded border border-blue-100 flex items-start text-xs mb-4">
            <Mail className="w-4 h-4 mr-2 mt-0.5 shrink-0" />
            <p>
              La contraseña será generada automáticamente por el sistema y
              enviada al correo electrónico del empleado.
            </p>
          </div>
